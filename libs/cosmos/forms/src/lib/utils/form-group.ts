...
import { dynamicFormArray } from './form-array';
  ...
  const addControlInterceptor = createFnInterceptor<FormGroup['addControl']>({
    argsInterceptor: (args) => {
      ...
      if (typeof existingValue !== 'undefined') {
        // console.log(`Preparing control '${name}' with value:`, existingValue);
      ...
      callInterceptor: (fn, thisArg, args) => {
        ...
        if (!returnValue) {
          ...
          if (existingValue && isFormGroupProp) {
            // console.log(`Creating control for '${name}' with value:`, existingValue);
...
export function defaultCreateControlForValue(
  ...
  console.log('Creating control for', propPath, existingValue);
  if (typeof existingValue !== 'object') {
    return new FormControl();
  }
  if (Array.isArray(existingValue)) {
    return dynamicFormArray(new FormArray([]), (itemValue: any) =>
