...
import { generateProjectLint, addLintFiles } from '@nrwl/workspace/src/utils/lint';
    ...
    (host: Tree, context: SchematicContext) => {
      ...
      return updateJsonInTree('tsconfig.json', json => {
        ...
        c.paths[`@${nxJson.npmScope}/${options.projectDirectory}`] = [
          `libs/esp/${options.projectDirectory}/src/index.ts`
        ];
        return json;
      })(host, context);
    }
...
function normalizeOptions(options: Schema): NormalizedSchema {
  ...
  const projectRoot = `libs/esp/${projectDirectory}`;
